"""
yaptype server manager. Start, stop, and check the ASR server.

Usage:
    uv run --no-sync python yapctl.py start       # Start server in background
    uv run --no-sync python yapctl.py stop         # Stop the server
    uv run --no-sync python yapctl.py status       # Check if server is running
    uv run --no-sync python yapctl.py restart      # Restart the server
"""
...
def cmd_start(args):
    """Start the ASR server in the background."""
    existing_pid = read_pid()
    if existing_pid is not None:
        if is_port_open(args.host, args.port):
            print(f"Server already running (PID {existing_pid}, port {args.port})")
            return
        else:
    ...
    print("\nTimeout waiting for server to start. Check logs:")
    print(f"  {LOG_FILE}")
...
def main():
    parser = argparse.ArgumentParser(
        description="yaptype server manager",
